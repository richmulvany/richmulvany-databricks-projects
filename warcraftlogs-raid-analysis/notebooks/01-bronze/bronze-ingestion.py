...
def save_json(subfolder: str, filename: str, payload: dict):
    ...
    with open(path, "w") as f:
        json.dump({
            "__metadata__": {"report_id": report_id, "report_start": meta["start_iso"]},
            **payload
        }, f)
...
def fetch_fights():
    ...
    save_json("fights", f"{report_id}_fights_{datetime.utcnow():%Y%m%dT%H%M%S}.json", data)
...
def fetch_actors():
    ...
    data = gql_query(q, {"code": report_id})
    save_json("actors", f"{report_id}_actors_{datetime.utcnow():%Y%m%dT%H%M%S}.json", data)
...
def fetch_tables():
    fights = gql_query(
        '''query($c:String!){ reportData{ report(code:$c){ fights{id} }}}''', {"c": report_id}
    )["reportData"]["report"]["fights"]
    for fight in fights:
        fid = fight["id"]
...
def fetch_world_data():
    ...
    save_json(
        "world_data",
        f"world_data_{datetime.utcnow():%Y%m%dT%H%M%S}.json",
        data
    )
