...
with st.spinner("Loading data…"):
    hps_df = load_csv("player_hps.csv")
    ranks_df = load_csv("ranks_hps.csv")
    details_df = load_csv("player_details.csv")
    players_df = load_csv("guild_roster.csv")
    CLASS_COLOURS = load_json("class_colours.json")

if difficulty != "all":
    hps_df = hps_df[hps_df["raid_difficulty"] == difficulty]
    valid_reports = hps_df["report_id"].unique()
...
available_raids = sorted(hps_df["raid_name"].dropna().unique().tolist())
...
if selected_raid != "all raids":
    hps_df = hps_df[hps_df["raid_name"] == selected_raid]
    valid_reports = hps_df["report_id"].unique()
    ranks_df = ranks_df[ranks_df["report_id"].isin(valid_reports)]

available_bosses = sorted(hps_df["boss_name"].dropna().unique().tolist())
...
if selected_boss != "all bosses":
    hps_df = hps_df[hps_df["boss_name"] == selected_boss]
...
# -------------------------------------------------------------------
# Aggregation for parse percent and hps by player
# -------------------------------------------------------------------
...
bar_chart = (
    ...
    .encode(
        ...
        tooltip=[
            ...
            alt.Tooltip("avg_hps:Q", title="avg hps"),
...
st.subheader("healing vs parse overview")
scatter = (
    alt.Chart(chart_df)
    .mark_circle(size=60, opacity=1.0)
    .encode(
        x=alt.X("avg_hps:Q", title="average hps"),
        y=alt.Y(f"{parse_field}:Q", title=parse_label),
        color=alt.Color(
            "player_class_lower:N",
            scale=alt.Scale(
...
st.subheader("player hps and parse summary")
display_df = chart_df[
    [
        "player_name",
        "player_class",
